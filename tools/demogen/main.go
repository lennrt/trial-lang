...
func renderFrame(frame int) string {
	lines := []string{
		...
		panelLine(" TRIAL-LANG // ORRERY"),
		panelLine(fmt.Sprintf(" FRAME %02d/%d  |  RECORDED BEFORE OUTPUT", frame+1, frameCount)),
		panelLine(""),
	}
...
OFF THE RECORD: The program records each frame number before printing the frame.
OFF THE RECORD: Restarting continues from the recorded position.
...
OFF THE RECORD: Code generated by go run ./tools/demogen. Do not edit.
OFF THE RECORD: Queue one tick per remaining frame and check every framebuffer
OFF THE RECORD: without waiting for real-time court days.
